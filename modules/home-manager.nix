...
in
{
  options.services.openchamber = {
    ...
    dataDir = mkOption {
      type = types.str;
      default = "${config.home.homeDirectory}/.config/openchamber";
      defaultText = literalExpression "\${config.home.homeDirectory}/.config/openchamber";
  ...
  config = mkIf cfg.enable {
    systemd.user.services.openchamber = {
      Unit = {
        Description = "OpenChamber Web Interface Daemon";
